// apps/mobile/src/__tests__/network.test.ts
//
// Ported 1:1 from apps/wallet/src/hardware/network.test.ts (vitest -> Jest).
import {
  coinTypeFor,
  defaultDerivationPath,
  coinIdFor,
  parsePath,
} from "../hardware/network";
...
describe("coinTypeFor", () => {
  ...
  it("returns 0 for betanet (mainnet fork)", () => {
    expect(coinTypeFor("betanet" as NetworkId)).toBe(0);
  });
...
describe("coinIdFor", () => {
  ...
  it("returns 'btc' for betanet (mainnet fork)", () => {
    expect(coinIdFor("betanet" as NetworkId)).toBe("btc");
  });
